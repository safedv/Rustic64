...
pub fn ldr_module(module_hash: u32) -> *mut u8 {
    ...
    let peb_ldr_data_ptr = unsafe { (*peb).loader_data as *mut PebLoaderData };
    ...
    while unsafe { !(*module_list).dll_base.is_null() } {
        ...
        if module_hash == dbj2_hash(dll_name_slice) {
            return unsafe { (*module_list).dll_base as _ }; // Return the base address of the module if the hash matches
        }
...
pub fn ldr_function(module_base: *mut u8, function_hash: usize) -> *mut u8 {
    ...
    let number_of_functions = unsafe { (*export_directory).number_of_functions };
    let array_of_names =
    ...
    for i in 0..number_of_functions {
        let name_addr = unsafe { module_base.offset(names[i as usize] as isize) } as *const i8;
        let name_len = get_cstr_len(name_addr as _); // Get the length of the function name
        let name_slice: &[u8] = unsafe { core::slice::from_raw_parts(name_addr as _, name_len) };
        ...
        if function_hash as u32 == dbj2_hash(name_slice) {
            ...
            return unsafe { module_base.offset(functions[ordinal] as isize) } as *mut u8;
        }
    }
